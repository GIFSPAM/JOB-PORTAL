  ...
  const renderRoleFields = () => {
    ...
    if (role === 'employer') return (
      <>
        ...
        <AuthField label="Company Phone (Optional)" icon={Phone}     type="tel" placeholder="+1 234 567 890"  value={companyPhone}    onChange={e => setCompanyPhone(e.target.value.toLowerCase())} />
      </>
    );

    // admin
    return <AuthField label="Admin Secret Key" icon={Key} type="password" placeholder="Enter security key" value={secretKey} onChange={e => setSecretKey(e.target.value)} required />;
  };
